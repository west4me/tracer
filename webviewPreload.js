...
document.addEventListener('keydown', (event) => {
    if (event.key === 'Escape') {
        ...
        ipcRenderer.sendToHost('shortcut-triggered', 'Escape');
    }
});
...
contextBridge.exposeInMainWorld('fullPageCapture', {
    capture: async () => {
        ...
        try {
            ...
            const steps = Math.ceil(fullHeight / viewportHeight);
            ...
            for (let i = 0; i < steps; i++) {
                ...
                // Wait for rendering and possible lazy-loaded images
                await new Promise(r => setTimeout(r, 300));

                // Use html2canvas for this section
                const dataUrl = await new Promise(resolve => {
                    ...
                    html2canvas.onload = async () => {
                        const result = await window.html2canvas(document.documentElement, {
                            allowTaint: true,
                            useCORS: true,
                            logging: false,
                            x: 0,
                ...
                // Draw this section to our full canvas
                tempImg.src = dataUrl;
                await new Promise(r => tempImg.onload = r);
                ctx.drawImage(tempImg, 0, currentScroll);
            }

            // Restore original scroll position
            window.scrollTo(originalScroll.x, originalScroll.y);

            // Return the full canvas as base64
